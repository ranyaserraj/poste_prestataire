...
export default class MissionCreateMedecinModalCopy extends LightningElement {
  @track isOpen = false;
  ...
  /** Priorité gate Apex ; sinon record type « Médecin » trouvé en LDS (jamais defaultRecordTypeId). */
  _missionRtFromGate = null;
  _ldsMissionMedecinRtId = null;
  ...
  /** Bouton + formulaire visibles uniquement pour un Account record type Médecin */
  ...
  get canShowEntry() {
    if (this.prestataireAccountId === null && this.accountRecordTypeDevNameApex === null) {
      return false;
    }
    return this.isMedecinAccountApex;
  }
  ...
  /**
   * Libellé picklist Branche (sinon valeur), sans accents — le combobox stocke la value API.
   */
  get _selectedBrancheNorm() {
    const controlling = this.picklistByRt?.picklistFieldValues?.Branche__c?.values;
    if (Array.isArray(controlling) && this.branche) {
      const entry = controlling.find(
        (v) => v?.value === this.branche || stripDiacritics(v?.value || '') === stripDiacritics(this.branche)
      );
      const text = entry?.label || entry?.value || this.branche;
      return stripDiacritics(text);
    }
    return stripDiacritics(this.branche || '');
  }

  /**
   * Libellé picklist Nature (sinon valeur), sans accents — le combobox stocke la value API, pas le libellé affiché.
   */
  get _selectedNatureNorm() {
    const values = this.picklistByRt?.picklistFieldValues?.Nature_Mission__c?.values;
    if (Array.isArray(values) && this.nature) {
      const entry = values.find(
        (v) => v?.value === this.nature || stripDiacritics(v?.value || '') === stripDiacritics(this.nature)
      );
      const text = entry?.label || entry?.value || this.nature;
      return stripDiacritics(text);
    }
    return stripDiacritics(this.nature || '');
  }

  get isSante() {
    const b = this._selectedBrancheNorm;
    return b === 'sante' || b.includes('sante');
  }
  ...
  get isAT() {
    ...
    if (!b) return false;
    if (b === 'at') return true;
    if (b.includes('accident') && b.includes('travail')) return true;
    return false;
  }

  _syncMissionRecordTypeId() {
    const next = this._missionRtFromGate || this._ldsMissionMedecinRtId || null;
    if (this.missionRecordTypeId !== next) {
      this.missionRecordTypeId = next;
    }
    // eslint-disable-next-line no-console
    console.log(
      'missionRecordTypeId (sync):',
      this.missionRecordTypeId,
      'fromGate:',
      this._missionRtFromGate,
      'fromLdsMed:',
      this._ldsMissionMedecinRtId
    );
  }
  ...
  wiredMissionInfo({ data, error }) {
    ...
    const med = Object.values(rts).find((rt) => {
      const dn = (rt?.developerName || '').toLowerCase();
      if (dn === 'medecin' || dn.includes('medecin')) return true;
      const nameKey = stripDiacritics(rt?.name || '');
      return nameKey === 'medecin' || nameKey.includes('medecin');
    });
    this._ldsMissionMedecinRtId = med?.recordTypeId || null;
    this._syncMissionRecordTypeId();
  }
  ...
  wiredGateInfo({ data }) {
    if (!data) return;
    // eslint-disable-next-line no-console
    console.warn('[medecinNouvellesMission-copy] gateInfo raw:', JSON.stringify(data));
    ...
    // eslint-disable-next-line no-console
    console.warn('[medecinNouvellesMission-copy] branches raw:', this.accountBranchesRaw);
    const gateRt = data.missionMedecinRecordTypeId;
    this._missionRtFromGate = gateRt || null;
    if (gateRt) {
      // eslint-disable-next-line no-console
      console.warn('[medecinNouvellesMission-copy] missionMedecinRecordTypeId (gate):', gateRt);
    }
    this._syncMissionRecordTypeId();
    this.syncBrancheFromAccount();
  }
  ...
  get natureOptions() {
    ...
    const msb = values.filter((v) => this._dependentPicklistApplies(v?.validFor, key, 'msb'));
    if (msb.length) {
      return msb.map((v) => ({ label: v.label, value: v.value }));
    }
    const lsb = values.filter((v) => this._dependentPicklistApplies(v?.validFor, key, 'lsb'));
    return lsb.map((v) => ({ label: v.label, value: v.value }));
  }

  /**
   * Picklist dépendante (Nature ← Branche) : exige un validFor exploitable.
   * Sans cela, l ancienne logique acceptait toutes les valeurs sans bitset → natures hors branche.
   */
  _dependentPicklistApplies(validFor, controllerIndex, mode) {
  ...
  get isSanteVacationSiege() {
    const n = this._selectedNatureNorm;
    return this.isSante && n.includes('vacation') && n.includes('siege');
  }

  get isSanteVacationSite() {
    const n = this._selectedNatureNorm;
    return this.isSante && n.includes('vacation') && n.includes('site');
  }

  get isATControle() {
    const n = this._selectedNatureNorm;
    return this.isAT && n.includes('controle') && (n.includes('medic') || n.includes('medical'));
  }

  get isATVacation() {
    if (!this.isAT || this.isATControle) return false;
    const n = this._selectedNatureNorm;
    if (!n.includes('vacation')) return false;
    if (n.includes(' sur ')) return false;
    return true;
  }
