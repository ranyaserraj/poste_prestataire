...
import getGateInfo from '@salesforce/apex/MissionCreateMedecinGateController.getGateInfo';
...
export default class MissionCreateMedecinModal extends LightningElement {
  // In Experience Builder, the running user context often isn't a Médecin portal user.
  // We detect Builder/preview URLs to allow UI preview without affecting runtime behavior.
  ...
  get isMedecinAccount() {
    ...
    return rt?.developerName === 'Medecin';
  }

  get isMedecinAccountApex() {
    return this.accountRecordTypeDevNameApex === 'Medecin';
  }
  ...
  get canShowEntry() {
    return this.isBuilderPreview || this.canCreateMission;
  }

  connectedCallback() {
    this.isBuilderPreview = this.detectBuilderPreview();
  }

  detectBuilderPreview() {
    try {
      const href = String(window?.location?.href || '').toLowerCase();
      const path = String(window?.location?.pathname || '').toLowerCase();
      // common Experience Builder / preview markers
      return (
        href.includes('livepreview') ||
        href.includes('sitepreview') ||
        href.includes('experiencebuilder') ||
        path.includes('/builder') ||
        href.includes('buildercontext')
      );
    } catch {
      return false;
    }
  ...
  computeBrancheOptionsList() {
    const values = this.picklistByRt?.picklistFieldValues?.Branche__c?.values;
    if (!Array.isArray(values)) return [];
    // Back to previous behavior: show all branches from Mission record type picklist
  ...
  syncBrancheFromAccount() {
    // No-op for old picklist behavior (no filtering/auto-selection)
    return;
  }

  get isSante() {
    return this.branche === 'Santé';
  }

  get isAT() {
    return this.branche === 'AT';
  }

  @wire(getObjectInfo, { objectApiName: MISSION_OBJECT_SCHEMA })
  wiredMissionInfo({ data }) {
    if (!data) return;
    const rts = data.recordTypeInfos || {};
    const med = Object.values(rts).find((rt) => rt?.name === 'Médecin' || rt?.name === 'Medecin');
  ...
  wiredAccountRecord({ data }) {
    ...
    if (!data) return;
    this.accountBranchesRaw = getFieldValue(data, ACCOUNT_BRANCHES) || '';
    this.accountRecordTypeId = getFieldValue(data, ACCOUNT_RECORD_TYPE_ID) || null;
    this.syncBrancheFromAccount();
  }
  ...
  get natureOptions() {
    ...
    // Fallback: compute controller index from the controlling field order.
    // This fixes cases where controllerValues keys don't match the actual value/label.
    ...
    // validFor is a base64 bitset. Filter by controller index.
    return values
  ...
  isValidForController(validFor, controllerIndex) {
    ...
    try {
      ...
      // UI API can return:
      // - validFor as base64 bitset (Metadata API style)
      // - validFor as array of controlling indexes (UI API picklist-values endpoint)
      ...
      // Bits are stored most-significant-bit first
  ...
  get isSanteVacationSiege() {
    return this.isSante && this.nature === 'Vacation sur siège';
  }

  get isSanteVacationSite() {
    return this.isSante && this.nature === 'Vacation sur site';
  }

  get isATControle() {
    return this.isAT && this.nature === 'Contrôle médicale';
  }

  get isATVacation() {
    return this.isAT && this.nature === 'Vacation';
  }
  ...
  get hInfoStepClass() {
    return `hStep ${this.currentStep === 1 ? 'active' : this.isInfoStepComplete ? 'complete' : 'future'}`;
  }
  get hPersonsStepClass() {
    return `hStep ${this.currentStep === 2 ? 'active' : this.isPersonsStepComplete ? 'complete' : 'future'}`;
  }
  get hLineClass() {
    return `hLine ${this.isInfoStepComplete ? 'complete' : ''}`;
  }
  ...
  async save() {
    this.errorMessage = '';
    if (!this.canCreateMission && !this.isBuilderPreview) {
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Accès refusé',
    ...
    try {
      ...
      if (this.heureDebut) fields.Heure_debut_passage__c = this.heureDebut;
      if (this.heureFin) fields.Heure_fin_passage__c = this.heureFin;
      if (this.nombreDossiers !== '' && this.nombreDossiers !== null && this.nombreDossiers !== undefined) {
